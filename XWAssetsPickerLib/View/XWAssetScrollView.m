...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        __weak XWAssetScrollView *weakSelf = self;
...
{
    UIImage *image   = [UIImage imageFromAssetBundle:@"asset_videoplay_btn"];
...
@interface XWAssetEditScrollView () <UIScrollViewDelegate>
...
@implementation XWAssetEditScrollView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        __weak XWAssetEditScrollView *weakSelf = self;
...
- (void)displayImage
{
    if (_editAsset && _editAsset.isPhoto)
    {
        // scale image properly
        ALAsset *asset = self.editAsset;
        CGFloat scale  = 1.0f;
